...
def LanderTest():
    ...
    #for _ in range(4000):
    while (True):
        observation, reward, terminated, truncated, info = env.step(np.random.choice(4, p=ActionP(observation)))
        if terminated or truncated:
            observation, info = env.reset()
    env.close()

def ActionP(obs):
    #x, y, dx, dy, angle, da, lLeg, rLeg
    weigths = [
        [0, 0, 0, 0, 0, 0, 100, 100], #nothing
        [5, 0, 10, 0, -40, -20, 0, 0], #left
        [0, -10, 0, -40, 0, 0, 0, 0], #main
        [-5, 0, -10, 0, 40, 20, 0, 0], #right
    ]
    bias = [0, 0, 0, 0]

    p = np.matmul(weigths, obs) + bias
    p -= p.min()
    p /= p.sum() + 1e-20
    print(p)
    return p
